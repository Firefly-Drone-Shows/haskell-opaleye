...
productQueryArr f = QueryArr (\at ->
                                let (b, pq, t') = f at
                                in (b, PQ.primQueryArr (PQ.APrimQuery (pure pq)), t'))

leftJoinQueryArr :: ((a, Tag) -> (b, HPQ.PrimExpr, PQ.PrimQuery, Tag)) -> QueryArr a b
leftJoinQueryArr f = QueryArr (\at ->
                                 let (a1, cond, primQuery', t1) = f at
                                 in (a1, PQ.primQueryArr (PQ.ALeftJoin cond (pure primQuery')), t1))
...
mapPrimQuery :: PQ.PrimPrimQueryArr -> SelectArr a b -> SelectArr a b
mapPrimQuery f sa =
  QueryArr ((\(b, pqf, t) -> (b, pqf <> PQ.primQueryArr f, t)) . runQueryArr sa)
...
runSimpleQueryArr f = (\(b, pqf, t) -> (b, PQ.toPrimQuery pqf, t))
                      . runQueryArr f
